...
async def analyze_path(request: AnalyzeRequest) -> JSONResponse:
    try:
        result = analyze_from_request(image_path=request.image_path)
...
async def analyze_url(request: AnalyzeRequest) -> JSONResponse:
    try:
        result = analyze_from_request(image_url=request.image_url)
        return JSONResponse(content=result.model_dump())
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def analyze_object(request: AnalyzeRequest) -> JSONResponse:
    try:
        result = analyze_from_request(bucket=request.bucket, object_key=request.object_key)
        return JSONResponse(content=result.model_dump())
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
